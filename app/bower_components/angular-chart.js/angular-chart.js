(function (factory) {
  ...
  function ChartJsFactory (ChartJs, $timeout) {
      ...
      return {
        ...
        link: function (scope, elem/*, attrs */) {
          ...
          scope.$watch('data', function (newVal, oldVal) {
            ...
            if (! chartType) return;
            if (chart) {
              if(!addNewData(newVal, oldVal, chart)){
                return;
              }
    ...
    function addNewData (newVal, oldVal, chart) {
      var stillOtherChanges = false;
      if(newVal.length !== oldVal.length) stillOtherChanges = true;
      for(var i = 0; i < newVal.length; i++){
        var row = newVal[i];
        for(var j = 0; j < row.length; j++){
          var element = row[j];
          if((j < (oldVal[i].length-1)) && element !== oldVal[i][j]) stillOtherChanges = true;

          if(j > (oldVal[i].length-1)){
            chart.addData([element]);
          }
        }
      }
      return stillOtherChanges;
    }
